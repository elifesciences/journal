...
# Retries a command up to MAX_ATTEMPTS times, sleeping DELAY seconds between attempts.
# Usage: retry "some command" MAX_ATTEMPTS DELAY
retry() {
    local cmd="$1"
    ...
    until eval "$cmd"; do
        if (( attempt >= max_attempts )); then
            echo "\"$cmd\" failed after $max_attempts attempts" >&2
            return 1
        fi
        echo "\"$cmd\" failed (attempt $attempt/$max_attempts), retrying in ${delay}s..." >&2
        sleep "$delay"
        (( attempt++ ))
    done
}
