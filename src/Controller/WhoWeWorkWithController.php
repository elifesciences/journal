...
final class WhoWeWorkWithController extends Controller
{
    ...
    private function toImageLinks(array $items) : array
    {
        return array_map(function (array $item) : ImageLink {
            $sources = [];
            if ($item['svg']) {
                $sources[] = ['srcset' => $this->get('puli.url_generator')->generateUrl("/elife/journal/images/logos/{$item['filename']}.svg"), 'type' => 'image/svg+xml'];
            }
            $sources[] = ['srcset' => sprintf('%s 200w, %s 400w', $this->get('puli.url_generator')->generateUrl("/elife/journal/images/logos/{$item['filename']}-lo-res.webp"), $this->get('puli.url_generator')->generateUrl("/elife/journal/images/logos/{$item['filename']}-hi-res.webp")), 'type' => 'image/webp'];
            ...
            return new ImageLink(
                ...
                new Picture(
                    $sources,
                    new Image(
                        $this->get('puli.url_generator')->generateUrl("/elife/journal/images/logos/{$item['filename']}-lo-res.png"),
                        [
                            400 => $this->get('puli.url_generator')->generateUrl("/elife/journal/images/logos/{$item['filename']}-hi-res.png"),
                        ],
                        $item['name']
                    )
                )
