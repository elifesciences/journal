...
final class SearchContext extends Context
{
    ...
    public function iShouldSeeTheMostRelevantResultsFor(int $number, string $keyword)
    {
        ...
        $this->spin(function () use ($number, $articles) {
            $this->assertSession()->elementsCount('css', '.message-bar:contains("'.count($articles).' results found") + .highlights > .listing-list > .listing-list__item', $number);

            for ($i = 0; $i < $number; ++$i) {
                $this->assertSession()->elementContains(
                    'css',
                    '.message-bar:contains("'.count($articles).' results found") + .highlights > .listing-list > .listing-list__item:nth-child('.($i + 1).')',
                    $articles[$i]['title']
                );
            }
        });
    ...
    public function iShouldSeeTheMostRelevantResultsAboutWithTheMSAs(int $number, string $keyword, string $subject1, string $subject2 = null)
    {
        ...
        $this->spin(function () use ($number, $articles) {
            $this->assertSession()->elementsCount('css', '.message-bar:contains("'.count($articles).' results found") + .highlights > .list-heading + .listing-list > .listing-list__item', $number);

            for ($i = 0; $i < $number; ++$i) {
                $this->assertSession()->elementContains(
                    'css',
                    '.message-bar:contains("'.count($articles).' results found") + .highlights > .list-heading + .listing-list > .listing-list__item:nth-child('.($i + 1).')',
                    $articles[$i]['title']
                );
            }
        });
